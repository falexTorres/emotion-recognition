...
images = np.load(join(SAVE_DIRECTORY, SAVE_DATASET_IMAGES_TEST_FILENAME))
labels = np.load(join(SAVE_DIRECTORY, SAVE_DATASET_LABELS_TEST_FILENAME))
...
for i in xrange(images.shape[0]):
    # print tflearn.variables.get_value(result[0])
    # if result is not None:
        # print result[0]
    # pridectList.append(result)
    # while True:
    #     if cv2.waitKey(1) & 0xFF == ord('q'):
    #         break
    result = network.predict(images[i])
    data[np.argmax(labels[i]), result[0].index(max(result[0]))] += 1
    pridectList.append(result[0]) 
	#print x[i], ' vs ', y[i]
...
c = plt.pcolor(data, edgecolors = 'k', linewidths = 4, cmap = 'Greens', vmin = 0.0, vmax = 1.0)
...
# print pridectList
np.save('testPrediction.npy',pridectList)
